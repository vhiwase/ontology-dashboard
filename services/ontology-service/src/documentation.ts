...
const PLATFORM_PAGES: Document[] = [
	{
		...
		title: "Simulated execution data",
		category: "Data quality",
		summary:
			"Which figures on this platform are measured and which are generated by a seeded simulation.",
		sections: [
			{
			...
			},
			{
				...
				body:
					"The snapshot contains no execution actuals: no recorded arrivals, no transit " +
					"times, no distances and no carrier assignments. On-time performance, dwell, " +
					"transit hours, cost per kilometre and transport cost are generated by a seeded " +
					"simulation (PIPELINE_SIM_SEED) so the metrics are answerable. They are " +
					"internally consistent and repeatable, and they are not observations. Treat " +
					"absolute values as illustrative and comparisons between them as directional.",
			},
			{
				title: "How to tell the difference",
				body:
					"Every metric carries dependsOnSimulation, and metrics built on simulated data " +
					"carry a coverage note naming exactly what was generated. The data coverage view " +
					"reports the split per metric area: how many rows came from the source and how " +
					"many were rated by the simulation.",
			},
			{
				title: "Refusing simulated data outright",
				body:
					"Setting ALLOW_SIMULATED_DATA=false makes the platform refuse rather than caveat. " +
					"Metrics, dashboard widgets and actions that depend on tms_sim return HTTP 409 " +
					"with an explanation instead of a number. This is the intended production " +
					"setting: a caveat is a convention, and conventions get quoted out of context.",
			},
		],
	},
	{
